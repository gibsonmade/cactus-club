...
export function VenueRoutePage({ venueId }: { venueId: string }) {
    ...
    return (
      <main className="min-h-screen bg-night px-4 py-6 text-bone">
        <div className="glass-panel mx-auto grid min-h-[80vh] max-w-5xl place-items-center rounded-[2rem] p-8 text-center">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-neon">Cactus Club</p>
            <h1 className="mt-4 font-display text-5xl leading-none">Venue not found.</h1>
            <Link className="mt-6 inline-flex rounded-full bg-bone px-4 py-2 text-xs font-black text-ink" href="/places">
              Back to places
            </Link>
          </div>
        </div>
  ...
  return (
    <main className="min-h-screen bg-night pb-16 text-bone">
      <div className="mx-auto w-full max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
        <motion.section
          ...
          className="space-y-8"
        >
          <section className="relative min-h-[440px] overflow-hidden rounded-[2rem] border border-white/10 bg-white/7 shadow-card">
            <SafeImage className="object-cover opacity-72" src={venue.imageUrl} fallbackSrc={fallbackImage} alt="" fill priority sizes="100vw" />
            <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(8,7,6,0.92)_0%,rgba(8,7,6,0.72)_48%,rgba(8,7,6,0.28)_100%),linear-gradient(180deg,rgba(8,7,6,0.08),rgba(8,7,6,0.82))]" />
            <div className="relative flex min-h-[440px] flex-col justify-between p-5 md:p-8">
              <div className="flex items-center justify-between gap-3">
                <Link className="rounded-full bg-bone px-4 py-2 text-xs font-black text-ink transition hover:bg-neon" href="/places">
                  Back to places
                </Link>
                <button className={cn("grid h-11 w-11 place-items-center rounded-full backdrop-blur-xl", saved ? "bg-neon text-ink" : "bg-black/35 text-bone")} onClick={toggleSavedVenue} aria-label={saved ? "Unsave venue" : "Save venue"}>
                  <Heart className={cn("h-5 w-5", saved && "fill-current")} />
                </button>
              </div>
              <div className="max-w-4xl">
                <p className="text-xs font-black uppercase tracking-[0.22em] text-neon">{venue.neighborhoodPersonality}</p>
                <h1 className="mt-3 font-display text-5xl font-black leading-[0.9] tracking-[-0.04em] text-balance md:text-8xl">{venue.name}</h1>
                ...
                <div className="mt-6 flex flex-wrap gap-2">
                  <span className="inline-flex items-center gap-2 rounded-full bg-white/12 px-3 py-2 text-xs font-bold"><MapPin className="h-4 w-4" />{venue.area === "Burbs" ? "Suburbs" : venue.area}</span>
                  <span className="rounded-full bg-white/12 px-3 py-2 text-xs font-bold">{venue.upcomingCount} upcoming</span>
                  {(venue.evergreenCount ?? 0) > 0 ? <span className="rounded-full bg-white/12 px-3 py-2 text-xs font-bold">{venue.evergreenCount} anytime ideas</span> : null}
                  ...
                  {venue.venueUrl ? (
                    <a className="inline-flex items-center gap-2 rounded-full bg-neon px-3 py-2 text-xs font-black text-ink" href={venue.venueUrl} target="_blank" rel="noreferrer">
          ...
          {venueEvergreenEvents.length ? (
            <section>
              <div className="mb-4">
                <p className="text-xs font-black uppercase tracking-[0.22em] text-mezcal">Always good here</p>
              ...
              <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                {venueEvergreenEvents.map((event) => (
                  <article className="glass-panel rounded-[1.5rem] p-5" key={event.id}>
                    <p className="text-xs font-black uppercase tracking-[0.18em] text-neon">{event.category}</p>
                    ...
                    {event.vibeTags.length ? (
                      <div className="mt-5 flex flex-wrap gap-2">
                        {event.vibeTags.slice(0, 4).map((tag) => (
                          <span className="rounded-full bg-white/10 px-3 py-1 text-[11px] font-bold text-bone/70" key={tag}>
                            {tag.replaceAll("-", " ")}
                          </span>
                        ))}
                      </div>
          ...
          <section>
            <div className="mb-4">
              <p className="text-xs font-black uppercase tracking-[0.22em] text-mezcal">Happening here</p>
...
function VenueEventCard({ event, onOpenDetails }: { event: EventItem; onOpenDetails: (event: EventItem) => void }) {
  return (
    <motion.article className="group relative rounded-[1.5rem] border border-white/10 bg-white/7 p-4 shadow-card" whileHover={{ y: -3 }} transition={{ duration: 0.22 }}>
      ...
      <div className="relative min-h-48 overflow-hidden rounded-[1rem]">
        ...
        <p className="absolute bottom-3 left-3 rounded-full bg-neon px-3 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-ink">{formatEventTime(event)}</p>
      </div>
      <h3 className="mt-4 text-left text-2xl font-black leading-tight transition group-hover:text-neon">{event.title}</h3>
